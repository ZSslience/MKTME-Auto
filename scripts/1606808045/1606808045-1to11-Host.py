...
def test_execution():
    ...
    log_write("INFO", "MSR Info: thread 0 0x9ff: %s, thread max 0x9ff: %s, thread 0 binary converted: %s" % (
        msr_9ff_core_0, msr_9ff_core_max, r_bin))
    log_write("INFO", "MSR bit 32 to 36: %s" % (r_bin[-36:-32]))
    result = [msr_9ff_core_0 == msr_9ff_core_max, r_bin[-36:-32] in msr_9ff_32_35]
    result_process(False not in result, "Check the value of CORE_MKTME_ACTIVATE MSR",
                   test_exit=False, is_step_complete=True)

    # Step 8: Disable AES-NI
    test_aesni_set("Disable")

    # Step 9: Disable TME
    test_mktme_set("Disable")

    # Step 10: Disable MKTME
    test_tme_set("Disable")
    test_bios_reset(complete=False)

    # Step 11: Check MSR 0x9FF again
    itp_ctrl("open")
    msr_9ff_core_0 = test_itp_msr(id=0x9ff, idx=0)
    msr_9ff_core_max = test_itp_msr(id=0x9ff, idx=(max_active_thread-1))
    itp_ctrl("close")

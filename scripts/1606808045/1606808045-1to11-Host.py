...
phy_address_width = int(utils.ReadConfig('1606808045', 'PHYSICAL_ADDRESS_WIDTH'), 16)
...
def test_execution():
    ...
    result = [msr_982_core_0 == msr_982_core_max, "1" in r_bin[-36:-32]]
    result_process(False not in result, "Check the value of IA32_TME_ACTIVATE MSR 0x982", test_exit=False, is_step_complete=True)

    itp_ctrl("open")
    msr_9ff_core_0 = test_itp_msr(id=0x9ff, idx=0)
    msr_9ff_core_max = test_itp_msr(id=0x9ff, idx=(max_active_thread-1))
    itp_ctrl("close")
    r_bin = "{0:064b}".format(msr_9ff_core_0)
    log_write("INFO", "MSR Info: thread 0 0x9ff: %s, thread max 0x9ff: %s, thread 0 binary converted: %s" % (msr_9ff_core_0, msr_9ff_core_max, r_bin))
    log_write("INFO", "MSR bit 32 to 36: %s" % (r_bin[-36:-32]))
    result = [msr_9ff_core_0 == msr_9ff_core_max, r_bin[-36:-32] in msr_9ff_32_35]
    result_process(False not in result, "Check the value of CORE_MKTME_ACTIVATE MSR", test_exit=False, is_step_complete=True)

    test_aesni_set("Disable")
    test_mktme_set("Disable")
    ...
    r_bin = "{0:064b}".format(msr_9ff_core_0)
    log_write("INFO", "MSR Info: thread 0 0x9ff: %s, thread max 0x9ff: %s, thread 0 binary converted: %s" % (msr_9ff_core_0, msr_9ff_core_max, r_bin))
    result = [msr_9ff_core_0 == msr_9ff_core_max, "1" not in r_bin]
    result_process(False not in result, "Check the value of CORE_MKTME_ACTIVATE MSR", test_exit=False, is_step_complete=True)

...
def test_execution():
    ...
    # Step 2: run cpuid(0x7, 0) and check ECX bit 13
    itp_ctrl("open")
    ...
    # Step 9: Save reset and boot to shell and run "memmap"
    test_bios_reset(complete=False)
    lpa.usb_to_sut()
    log_file_name = test_efi_command_run('memmap', 30, "Save, reset, boot to shell, run memmap", log=True)

    # Step 10: check MEMORY_CPU_CRYPTO
    lpa.usb_to_host()
    time.sleep(5)
    log_file = usb_drive_label + log_file_name
